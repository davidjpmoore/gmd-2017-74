...
contains
  ...
  subroutine CNAllocation (bounds, num_soilc, filter_soilc, num_soilp, filter_soilp )
   ...
   do fp=1,num_soilp
      ...
      ! modified dynamic f2 based on Litton et al. 2007
	  
      if (croot_stem(ivt(p)) == -1._r8) then
         f2 = (0.25-8e-05*annsum_npp(p))/(0.17+ 0.0001158*annsum_npp(p)) 
      else
         f2 = croot_stem(ivt(p))
      end if
	  ...
	  ! modified dynamic f3 based on Litton et al. 2007
	  
      if (stem_leaf(ivt(p)) == -1._r8) then
         f3 = (0.17+0.0001158*annsum_npp(p))/0.26
      else
         f3 = stem_leaf(ivt(p))
      end if
